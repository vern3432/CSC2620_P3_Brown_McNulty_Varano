...
public class FamilyDatabase {
    ...
    private static List<FamilyMember> fetchAttendeesForEvent(int eventId) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("SELECT * FROM EventAttendees WHERE event_id = ?");
    ...
    */
    private static FamilyMember fetchFamilyMemberById(int memberId) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("SELECT * FROM FamilyMembers WHERE member_id = ?");
    ...
    public static int addEvent(Date eventDate, String eventType, int memberId) {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO Event (event_id, event_date, event_type, member_id) VALUES (?, ?, ?, ?)");
            statement.setInt(1, eventId);
    ...
    */
    public static boolean addAttendeeToEvent(int eventId, int memberId) {
        try {
            PreparedStatement statement = connection
                    .prepareStatement("INSERT INTO EventAttendees (event_id, member_id) VALUES (?, ?)");
            statement.setInt(1, eventId);
    ...
    */
    public static ArrayList<FamilyMember> getAllDeceasedFamilyMembers() {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("SELECT * FROM FamilyMembers WHERE is_deceased = ?");
            ...
            while (resultSet.next()) {
                ...
                FamilyMember familyMember = new FamilyMember(id, name, birthDate, deathDate, isDeceased,
                        currentResidence);
    ...
    */
    public List<Event> getAllEvents() {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                String query = "SELECT * FROM EventAttendee WHERE event_id=?";

                PreparedStatement statement2 = connection.prepareStatement(query);
                statement2.setInt(1, eventId);
                ResultSet resultSet_attendees = statement2.executeQuery();
                while (resultSet_attendees.next()) {
                    Integer temp=resultSet_attendees.getInt("member_id") ;
                    event.addAttendee2(temp);
                }
                events.add(event);
    ...
    private static int getMemberIdByName(String name) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("SELECT member_id FROM FamilyMembers WHERE name = ?");
    ...
    // Relationships (member_id, related_member_id, relation_type) VALUES (?, ?,
    // ?)");
    // statement.setInt(1, memberId);
    ...
    private static void insertAddress(String[] values) {
        try {
            ...
            PreparedStatement statement = connection
                    .prepareStatement("INSERT INTO Addresses (city, member_id) VALUES (?, ?)");
    ...
    private void insertEvent(String[] values) {
        try {
            ...
            PreparedStatement statement = connection
                    .prepareStatement("INSERT INTO Event (event_date, event_type, member_id) VALUES (?, ?, ?)");
    ...
    */
    ...
    /// good end, past here is unkown if its even still needed. Likley it isnt, will
    /// have to clean up as We go
    private static void insertFamilyMember(String[] values) {
        try {
            PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO FamilyMembers (name, birth_date, death_date, is_deceased, current_residence, client_id) VALUES (?, ?, ?, ?, ?, ?)");
            statement.setString(1, values[0]);
    ...
    public static int insertClient(String clientName) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("INSERT INTO Clients(client_id, client_name) VALUES (?, ?)");
    ...
    public static int insertFamilyMember(String name, Date birthDate, Date deathDate, boolean isDeceased,
            String currentResidence, int clientId) {
    ...
    public static int insertAddress(String city, int memberId) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("INSERT INTO Addresses(address_id, city, member_id) VALUES (?, ?, ?)");
    ...
    public static String getClientName(int clientId) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("SELECT client_name FROM Clients WHERE client_id = ?");
            statement.setInt(1, clientId);
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
    ...
    public static String getMemberName(int memberId) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("SELECT name FROM FamilyMembers WHERE member_id = ?");
            statement.setInt(1, memberId);
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
    ...
    public static int getMemberIdByNameAndBirthDate(String name, Date birthDate) {
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("SELECT member_id FROM FamilyMembers WHERE name = ? AND birth_date = ?");
            statement.setString(1, name);
    ...
    public static void main(String[] args) {
        ...
        // insertFamilyMember("John Doe", new Date(), null, false, "New York",
        // clientId1);
        // if (memberId1 != -1) {
        // System.out.println("Family member inserted successfully. Member ID: " +
        // memberId1);
        // } else {
        // System.out.println("Failed to insert family member.");
        // }

        // insertFamilyMember("Jane Smith", new Date(), null, false, "Los Angeles",
        // clientId2);
        // if (memberId2 != -1) {
        // System.out.println("Family member inserted successfully. Member ID: " +
        // memberId2);
        ...
        // System.out.println("Event inserted successfully for " +
        // getMemberName(memberId1));

        // insertEvent(new Date(), "Wedding", memberId2);
        // System.out.println("Event inserted successfully for " +
        // getMemberName(memberId2));
        ...
        // if (relationshipId != -1) {
        // System.out.println("Relationship inserted successfully. Relationship ID: " +
        // relationshipId);
        // } else {
        ...
        // if (eventId != -1) {
        // System.out.println("Event inserted successfully for " +
        // getMemberName(memberId1) + ". Event ID: " + eventId);
        // } else {
